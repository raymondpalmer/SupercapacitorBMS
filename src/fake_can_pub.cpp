...
private:
  void tick() {
    ...
    // ---- 0x02F5：功率/剩余时间/循环次数/故障位 ----
    {
      ...
      uint16_t remain_min = (uint16_t)std::clamp((int)std::lround((soc_ / 100.0) * 10.0 / I * 60.0), 0, 65535); // 假设 10Ah

      pack_s16_be(&f.data[0], p_W);
      pack_u16_be(&f.data[2], remain_min);
      pack_u16_be(&f.data[4], 163); // cycle
